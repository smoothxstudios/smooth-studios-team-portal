...
export async function makePreview(bytes){
 for(const [edge,quality] of [[960,78],[640,65]]){
  const preview=await sharp(bytes,{limitInputPixels:80_000_000}).rotate().resize({width:edge,height:edge,fit:'inside',withoutEnlargement:true}).webp({quality}).toBuffer();
...
export async function backfillImagePreviews({origin,cookie,images}){
 const stats={created:0,failed:0,originalBytes:0,previewBytes:0};
 ...
 for(const image of images){
  try{
   ...
   const bytes=Buffer.from(await original.arrayBuffer()),preview=await makePreview(bytes);
  ...
  }catch(error){stats.failed++;console.log('Image preview skipped: '+error.message);}
 }
 return stats;
}
