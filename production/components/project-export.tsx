...
export function ExportDialog({scope,project,projects,onClose}:{scope:'current'|'all';project:Project|null;projects:ProjectSummary[];onClose:()=>void}){
 ...
 const [layout,setLayout]=useState('Shot List'),[order,setOrder]=useState('Shooting Order');
 ...
 <fieldset className="project-options"><legend>Include Sections</legend><p>Only enabled sections will appear in each production’s PDF.</p><div className="module-picker">{available.map(s=><label className="option-card" key={s.key}><Checkbox checked={sections.includes(s.key)} onCheckedChange={v=>setSections(v?[...sections,s.key]:sections.filter(k=>k!==s.key))}/><span><strong>{s.label}</strong></span></label>)}</div></fieldset><div className="export-options"><FormField label="PDF Organization"><Choice label="PDF Organization" value={mode} onChange={setMode} options={['One Combined PDF',scope==='current'?'Separate PDF Per Section':'Separate PDF Per Production']}/></FormField>{sections.includes('shots')&&<><FormField label="Shot List Layout"><Choice label="Shot List Layout" value={layout} onChange={setLayout} options={['Shot List','Shot Breakdown','Storyboard']}/></FormField>{layout==='Storyboard'&&<p className="export-note">Notes are editable in the downloaded PDF. PDF edits stay in that file.</p>}<FormField label="Shot Order"><Choice label="Shot Order" value={order} onChange={setOrder} options={['Shooting Order','Scene and Shot']}/></FormField></>}<label><Checkbox checked={images} onCheckedChange={v=>setImages(!!v)} disabled={!sections.includes('shots')}/>Include shot reference images</label></div><p className="export-note">Scripts include the text saved in the project. Uploaded documents appear as a file list; their contents remain in the dashboard.</p>
