...
export function ExportDialog({scope,project,projects,onClose}:{scope:'current'|'all';project:Project|null;projects:ProjectSummary[];onClose:()=>void}){
 ...
 const [layout,setLayout]=useState('First AD shot sheet'),[order,setOrder]=useState('Shooting order');
 ...
 <fieldset className="project-options"><legend>Include sections</legend><p>Only enabled sections will appear in each production’s PDF.</p><div className="module-picker">{available.map(s=><label className="option-card" key={s.key}><Checkbox checked={sections.includes(s.key)} onCheckedChange={v=>setSections(v?[...sections,s.key]:sections.filter(k=>k!==s.key))}/><span><strong>{s.label}</strong></span></label>)}</div></fieldset><div className="export-options"><Choice label="PDF organization" value={mode} onChange={setMode} options={['One combined PDF',scope==='current'?'Separate PDF per section':'Separate PDF per production']}/>{sections.includes('shots')&&<><Choice label="Shot list layout" value={layout} onChange={setLayout} options={['First AD shot sheet','Detailed shot breakdown']}/><Choice label="Shot order" value={order} onChange={setOrder} options={['Shooting order','Scene and shot']}/><p className="export-note">{layout==='First AD shot sheet'?'Landscape rows with checkboxes, timing, reference thumbnails, and room for take notes.':'A portrait breakdown with larger references and expanded shot details.'}</p></>}<label><Checkbox checked={images} onCheckedChange={v=>setImages(!!v)} disabled={!sections.includes('shots')}/>Include shot reference images</label></div><p className="export-note">Scripts include the text saved in the project. Uploaded documents appear as a file list; their contents remain in the dashboard.</p>
