...
export async function browserReferenceImage(id:string,headers:Record<string,string>){
 const response=await fetch('/api/images/'+id,{cache:'no-cache',headers,signal:AbortSignal.timeout(30000)});if(!response.ok)throw new Error('Reference unavailable');
