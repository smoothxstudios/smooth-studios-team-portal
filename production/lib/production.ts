...
export type ProductionData={stage:string;due:string;timezone:string;tasks:Task[];documents:ProductionDocument[];schedule:ScheduleBlock[];expenses:Expense[];budgetCents:number;callSheet:CallSheet};
...
export function defaultProduction():ProductionData{return {stage:"Pre-production",due:"",timezone:"America/New_York",tasks:[],documents:[],schedule:[],expenses:[],budgetCents:0,callSheet:{date:"",call:"",wrap:"",location:"",address:"",parking:"",contactName:"",contactPhone:"",safety:"",notes:""}};}
export function productionOf(p:{production?:ProductionData}){const fallback=defaultProduction();return {...fallback,...p.production,callSheet:{...fallback.callSheet,...p.production?.callSheet}};}
