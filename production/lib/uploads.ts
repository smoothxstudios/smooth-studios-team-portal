...
export async function uploadReference(file:File,projectId?:string,progress:(percent:number)=>void=()=>{}):Promise<Reference>{
 ...
 try{
  ...
  if(small){try{await fetch('/api/images/'+result.image.id+'?preview=1',{method:'PUT',headers:{...accountHeaders(),'Content-Type':small.type},body:small,signal:AbortSignal.timeout(8000)});}catch{/* The original remains available if its optional display copy cannot be saved. */}}
