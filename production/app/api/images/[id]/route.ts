import {api,db,bucket,identity,RequestError,PROJECT_ACCESS_SQL} from "@/lib/server";
export async function GET(request:Request,context:{params:Promise<{id:string}>}){return api(async()=>{
 ...
 const row=await db().prepare("SELECT i.mime FROM images i WHERE i.id=? AND (?=1 OR (i.project_id IS NULL AND i.owner=?) OR EXISTS (SELECT 1 FROM projects p WHERE "+PROJECT_ACCESS_SQL+" AND (p.id=i.project_id OR EXISTS (SELECT 1 FROM json_each(p.data,'$.shots') s, json_each(s.value,'$.references') r WHERE json_extract(r.value,'$.id')=i.id))))").bind(id,u.admin?1:0,u.id,u.id,u.admin?1:0,u.email).first<{mime:string}>();
