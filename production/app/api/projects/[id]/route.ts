...
export async function PUT(request:Request,context:Context){return api(async()=>{
 const u=await identity(request),{id}=await context.params,old=await requireProject(id,u),{teamAccountIds,...p}=await projectBody(request);
 if(teamAccountIds!==undefined)throw new RequestError('Use project team tagging to change access.',403);
