...
export async function GET(request:Request){return api(async()=>{
 const u=await identity(request);const rows=await db().prepare("SELECT p.id, p.title, p.updated_at, json_array_length(p.data,'$.shots') AS shots, json_extract(p.data,'$.client') AS client, json_extract(p.data,'$.date') AS date, json_extract(p.data,'$.production.stage') AS stage, json_extract(p.data,'$.production.due') AS due, json_extract(p.data,'$.production.modules') AS modules, json_array_length(p.data,'$.production.tasks') AS task_count, (SELECT COUNT(*) FROM json_each(p.data,'$.production.tasks') t WHERE json_extract(t.value,'$.status')='Done') AS tasks_done FROM projects p WHERE p.owner = ? OR ? = 1 OR EXISTS (SELECT 1 FROM project_members m WHERE m.project_id = p.id AND m.email = ?) ORDER BY p.updated_at DESC").bind(u.id,u.admin?1:0,u.email).all<any>();
 return json({projects:rows.results.map(x=>({id:x.id,title:x.title,updatedAt:x.updated_at,shots:x.shots,client:x.client,date:x.date,stage:x.stage||"Pre-production",due:x.due||"",modules:x.modules?JSON.parse(x.modules):ALL_MODULES,taskCount:x.task_count||0,tasksDone:x.tasks_done||0}))});
});}
