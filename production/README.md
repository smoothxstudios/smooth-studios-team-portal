...
The owner username is `smooth`. The initial existing team accounts are `akiva`, `jordyn`, and `rayne`. The deployment initializes missing accounts from the existing studio portal's protected password secrets; it never resets existing production passwords. The Production login offers Sign in and Sign up. New users can register with their name, a unique username, and a password of at least 12 characters, then enter their account immediately. Smooth can also create accounts, reset temporary passwords, and enable or disable accounts. Enabled team accounts can create their own productions and tag teammates on projects they create. They only see productions they created or are tagged in; Smooth can see every production. Owner-created accounts and reset passwords require a password change; people who sign up choose their own password and can proceed directly. Password changes sign out other devices. Disabling accounts revokes sessions. The owner's account cannot be disabled from the dashboard.

Better Auth 1.7.3 supplies scrypt password hashing, signed HTTP-only session cookies, origin protection, and database-backed login rate limiting. Project APIs enforce authorization on every request. Legacy ChatGPT headers are ignored. Account roles are determined by the immutable owner account ID; client profile fields cannot grant owner permissions. Owner-created accounts may include a contact email. Public registration uses username accounts without collecting email: the Worker generates the internal `.invalid` alias, strips submitted identity, role, verification, and access fields, and rejects aliases that could inherit an old project assignment. Reserved owner usernames cannot be registered. Sign-up is limited to five attempts per minute per connecting IP. Internal placeholder identifiers ending in `.invalid` are never used for messages. No invitations or emails are sent automatically.
...
Run `npm ci`, `npm run typecheck`, `npm run build`, then `npm test`. Tests run the actual Worker in Miniflare and exercise public sign-up, automatic sign-in, registration privilege isolation, duplicate usernames, password changes, account management, tag-restricted project viewing and editing, rate limiting, CSRF rejection, and shared files.
...
Account administration and the Studio Dashboard link are under Settings. Any enabled teammate can create a production and tag active teammates during creation. The project creator and Smooth can manage its team from Project overview or Crew & Tasks, and can delete the project. Tagged collaborators can edit project content but cannot change its team or delete the project. A read-only team directory supplies active accounts for tagging; account administration stays owner-only. Project ownership comes from the verified server session and cannot be changed through submitted project fields. Tags grant viewing, editing, file access, and PDF export permission even when Crew & Tasks is off. Team accounts only receive summaries and project data for productions they created or are tagged in; Smooth keeps access to all projects. Direct project, crew, file, and reference image requests enforce the same access rule, including images copied between projects. Inaccessible projects and images return 404 without disclosing their contents. Removing a tag revokes access on the next request; the creator and Smooth retain access through ownership. Open workspaces refresh assignments on focus and every 30 seconds and clear a revoked project and its open dialogs. Previously downloaded files cannot be recalled. Project updates, image and file uploads, and file deletion require an assignment, project ownership, or Smooth’s access. No notifications or invitations are sent by tagging.
...
PDF export can combine selected productions, produce one PDF per production, or export selected project sections separately. Enabled sections, written scripts, crew, schedules, call sheets, expenses, and optional reference images are included. Uploaded documents are listed by filename; their contents remain separate. The export runs in the signed-in browser and downloads authenticated references without sending project data to another service. The PDF engine and bundled fonts load only when exporting. The engine is a self-contained module at a stable, revalidated URL so existing tabs can load it after deployments; failed module loads can retry. HTML revalidates on refresh; missing assets return 404 rather than the app HTML. Font license is in `public/fonts/LICENSE.txt`.
...
## Loading and image previews

The home screen loads its verified account once and fetches only project summaries. Full shot data loads when opening a production. API reads have a bounded timeout, including response bodies, and retry once after transient connection failures; writes and sign-in requests are never automatically repeated. A failed session check clears unverified content and offers Try loading again.

Reference images use a small WebP display copy (up to 960 pixels and 256 KiB); enlargement and PDF export use the original bytes. Browsers create previews alongside new uploads. Preview failure never discards a successful original upload. During deployment, existing references are converted through the authenticated owner APIs only while the existing deployment credential still matches; originals stay unchanged. Failed conversions fall back to the original.

Image responses are private and must revalidate with the server before reuse. Authorization runs before any 304 response, so removing project access blocks cached requests as well. Account-specific URLs and ETags separate browser account changes. Indexed image/project references are backfilled by migration 0005 and maintained atomically by project insert/update/delete triggers. No project data or preview bytes are published as static assets. Deployment readiness checks include the current app script, PDF module, authenticated preview revalidation, and anonymous access rejection.
